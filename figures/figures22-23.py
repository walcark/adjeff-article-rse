...
def pct_improvement(rmse_other, rmse_king):
    """Positive = King is better."""
...
for idx, (ax, band) in enumerate(zip(axes_flat, bands)):
    df = load_band(band)
    ...
    ax.set_title(f"({'abcdefgh'[idx]}) {band}", fontsize=12 * tick_factor, pad=10)
    ax.scatter(
        x,
        y_unif,
        marker=markers["unif"],
    ...
    ax.tick_params(
        axis="both", which="major", width=1.5, length=6, labelsize=10 * tick_factor
    )
    ax.tick_params(
        axis="both", which="minor", width=1.0, length=3, labelsize=8 * tick_factor
    )
...
fig1.tight_layout()
fig1.savefig(OUTPUT_DIR / "figure22.png", dpi=150, bbox_inches="tight")
print("Saved figure22.png")

# ── Figure 2 : amélioration % de King vs longueur d'onde ────────────────────
...
ax2.tick_params(
    axis="both", which="minor", width=1.5, length=3, labelsize=8 * tick_factor
)
...
fig2.tight_layout()
fig2.savefig(OUTPUT_DIR / "figure23.png", dpi=150, bbox_inches="tight")
print("Saved figure23.png")
